...
// -- CUSTOM FORM TEMPLATES (Documentation/Resources merge) --
async function loadCustomTemplates() {
    try {
        const res = await fetch('/api/forms/templates/available', { credentials: 'include' });
        if (!res.ok) throw new Error('Failed to load custom forms');
        return await res.json();
    } catch (err) {
        console.error('Load custom templates error:', err);
        return [];
    }
}

function customCardHTML(t) {
    return `
        <div class="form-card">
            <div class="form-card-accent"></div>
            <div class="form-card-body">
                ...
                <div class="form-card-title">${t.title}</div>
                <div class="form-card-desc">${t.description || ''}</div>
                <button class="form-card-btn" onclick="event.stopPropagation(); openCustomForm(${t.id})">
                    Open
                    <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/>
                    </svg>
...
function openCustomForm(templateId) {
    const token = btoa(`scooters-custom-${templateId}-${Math.floor(Date.now() / 300000)}`);
    ...
    const tab = window.open(`/scooters/forms/fill.html?template_id=${templateId}&pt=${token}${locationParam}${adminParam}`, '_blank');
    if (tab) {
...
async function renderFormsPage() {
    ...
    grid.innerHTML += templates.filter(t => t.category === 'documentation').map(customCardHTML).join('');
}
...
async function renderResourcesPage() {
    ...
    const templates = await loadCustomTemplates();
    grid.innerHTML += templates.filter(t => t.category === 'resource').map(customCardHTML).join('');
}
